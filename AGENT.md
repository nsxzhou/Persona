...
- **大模型 (LLM) 测试规范**:
  - **Mock 测试优先**：验证状态机流转、接口逻辑与数据存储时，必须使用 Mock 数据生成器（如 `build_fake_analysis_report`）隔离真实的大模型网络调用，确保单测的确定性、零成本与高执行速度。
  - **Live 真实集成测试**：涉及 Prompt 准确性验证或大模型结构化输出（Structured Output）边界测试时，须发起真实调用。测试代码中严禁硬编码 API 密钥，必须通过 `PERSONA_TEST_PROVIDER_*` 等环境变量注入真实凭证。
  - **测试样本工程化**：进行诸如文风分析等长文本相关测试时，严禁在 Python 测试文件中硬编码大段字符串。具有代表性的长文本（如万字级别的小说片段）必须作为独立资源文件存放在本地（如 `api/tests/sample_novel.txt`），并通过 `pytest fixture` 动态加载读取，确保测试代码整洁且样本具备真实代表性。
