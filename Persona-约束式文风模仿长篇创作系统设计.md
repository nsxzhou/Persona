...
2. **风格实验室：TXT 上传 -> 输入判定 -> 分块分析 -> 聚合 -> 结构化报告 -> 风格摘要 -> 风格母 Prompt 包。**  
   **（已完成：深度分析闭环）** 当前已实现“单个 TXT -> 深度分析任务 -> 完整分析报告 -> 风格摘要 -> 风格母 Prompt 包 -> 风格档案 -> 项目挂载”的可用闭环，包含：
   ...
   - 基于已配置 Provider/模型的真实 LLM 多阶段分析，并改用结构化输出替代裸 JSON 解析
...
- 后端启动时拉起进程内单 Worker，通过 lease claim `pending` 任务；若发现陈旧 `running` 任务，会释放锁并恢复为可续跑的 `pending`
...
- LLM 输出已改为 Pydantic 结构化 schema 驱动，使用 `with_structured_output(..., method=\"json_schema\", strict=True)` 替代正则 JSON 解析
- 后端任务结果已稳定为 `analysis_meta / analysis_report / style_summary / prompt_pack`
...
- 进程内 Worker 负责 claim/lease、心跳更新、启动时的陈旧任务恢复，以及分析结果写回
...
- 所有 LLM 节点都使用 Pydantic schema 做结构化输出，`3.1` 到 `3.12` 的 section 完整性在 schema 层强校验
- 当前切片仍采用段落聚合的启发式策略，但最终产物已稳定为“分析元信息 + 结构化报告 + 风格摘要 + 风格母 Prompt 包”
...
当前数据库中实际落地的风格档案对象，已经升级为“报告 + 摘要 + Prompt 包”的组合体；报告和中间分析结构都由 Pydantic schema 校验，更接近下面的形态：
