...
为了彻底解决大模型的遗忘问题与“幻觉脏写”导致的世界观崩溃，系统采用**事件溯源与影子确认**相结合的“四表一图”架构：

### 1. 事件溯源记录（Event Sourcing）
数据库不直接存储实体的最终状态（如 `亚瑟.状态 = 死亡`），而是记录追加式的事件流日志（Append-Only Log）。例如 `[第5章事件: 亚瑟受重伤]`。
当前的世界状态是由历史事件流 Reduce 计算得出的。若大模型发生幻觉，只需回滚或删除特定的错误事件记录，即可实现完美的时间旅行，保证数据底座的绝对安全。

### 2. 影子状态确认（Shadow State / HITL）
由大模型在每章生成后的“后置钩子”提取出的状态变更，首先进入“暂存区（Shadow State）”。
作者在编辑器中会看到明确的提示（例如：“本章提取到 2 个状态变更：获得道具X、结识角色Y”）。只有经过人类显式确认（Confirm）的事件才会被追加到主数据库，坚决杜绝大模型直接脏写。

### 3. 核心数据结构（四表一图）
- **角色状态表**：基于事件流聚合出的当前地理位置、情绪值、随身物品、关系亲密度、当前境界。
- **时间线表**：故事内绝对时间、关键事件触发戳。
- **因果链表**：事件间的先决条件映射（如“得知秘密” -> “发起复仇”）。
- **伏笔网络表**：悬念 ID、抛出章节、预期回收章节、当前状态（活跃/逾期/已回收）。
- **知识图谱（KG）**：通过 Bootstrap 管线逆向导入或手动维护的实体-关系网络（角色/地点/组织/阵营），并支持打包为可复用的 Worldpack。

---

## 八、上下文注入引擎（Context Injection Engine）

为了在有限的上下文窗口中精准投喂信息，同时消除影响创作心流的手动打标签行为，系统采用**隐式实体识别与三温区图检索**架构：

### 1. 完全隐式注入（Implicit NER）
废弃需要手动编写的 `@DSL` 标签（如 `@Character:亚瑟`）。系统在生成正文前，自动对“本章大纲”和“上一章末尾文本”进行 NER（命名实体识别），静默锁定当前场景中出场的角色、地点、核心道具，并自动将其档案拉入上下文。

### 2. 三温区动态装载（Hot / Warm / Cold）
- **Hot（热区）**：当前章节的直接前文（前 1-2 章摘要 + 最后 2000 字原文）、本章核心出场角色状态。
- **Warm（温区）**：近期活跃的伏笔、隐式 NER 识别出的关联实体、悬而未决的冲突。
- **Cold（冷区）与 Graph-RAG**：放弃纯向量检索（FAISS），改用 **Graph-RAG**。顺着知识图谱的“边”进行拉取（如：识别到亚瑟出场，自动顺着图谱边拉取“亚瑟的宿敌”与“当前装备”）。保证拉取的是高逻辑关联的设定，而非简单的字面重合。

### 3. POV 视角信息过滤
在组装上下文后执行权限校验。标记当前的视点人物（POV），将该角色当前尚未获知的情报进行物理屏蔽。从根本上杜绝“主角全知全能”的叙事漏洞。

---

## 九、长篇工作流编排引擎（Workflow Orchestration Engine）

将长篇创作从“一次性黑盒生成”拆解为有序的、可审计的、可回滚的工程化流水线。

### 1. 双层分形状态机（Fractal State Machine）
全书放入单一 LangGraph 会导致状态对象极度膨胀（Token 爆炸）。系统拆分为：
- **MacroGraph（宏观图）**：负责全局规划（题材 -> 三幕结构 -> 卷大纲）。
- **MicroGraph（微观图）**：负责单章执行（单章大纲 -> 正文初稿 -> 润色）。
单章生成完成后，只向宏观图返回章节摘要和状态变更，实现长篇上下文的物理隔离。

### 2. 语义依赖追踪（Semantic Dependency Tracking）
摒弃极易引发缓存雪崩的纯 Hash 校验。采用语义依赖矩阵来决定章节是否需要重跑。
例如：修改了全局设定“魔法体系”，只有打上了 `[依赖: 魔法体系]` 标签的章节才会被标记为“Stale（需重审）”，而其他章节的缓存依然有效，极大提升修改与迭代的效率。

### 3. 独立 LLM 配置与节点检查点
- 大纲与推演节点使用强推理模型（如 Claude 3.5 Sonnet / Opus）。
- 润色节点使用对文风更敏感的模型。
- 每一个 StateNode 均有持久化检查点，支持任意节点的回滚与重跑。

---

## 十、质量控制与人机协同子系统（Quality Control Subsystem）

彻底摒弃“每次生成后叠加五层 LLM 强力清洗”的高昂且僵化的流程，确立**“防范优于清洗”**的策略：

### 1. 风格前置（Shift-Left Styling）
消除“AI 味”的最佳时机是在起草（Drafting）阶段。深度依赖 Style Lab 产出的【风格母 Prompt】与【Few-shot 样本】。将作者个人的遣词造句习惯、句法节奏作为硬性约束在初稿生成时直接注入，避免后置的过度清洗将文字变成毫无生气的“科学怪人”。

### 2. 弹性多维路由（Elastic QC Routing）
不再对每一章执行全量评审：
- **Fast Path（日常过渡章）**：仅做轻量级的“逻辑与事实一致性”校验。
- **Deep Path（高潮关键章）**：开启完整的“情感曲线校准”与“五维深度评审”（逻辑、角色、伏笔、文笔、读者体验）。

### 3. Copilot 批注模式（类似代码 Linter）
AI 评审的结果不强制自动重写正文，而是在 Zen Editor 中以“划线批注”的形式展现。
例如：“此处可能发生 OOC”或“此处违背了第 5 章的设定”。作者可以点击“一键修复”由 AI 改写，也可以手动修改。始终确保人类对作品的最终掌控权。
