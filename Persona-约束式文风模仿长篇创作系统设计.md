...
2. **风格实验室：TXT 上传 -> 输入判定 -> 分块分析 -> 聚合 -> 结构化报告 -> 风格摘要 -> 风格母 Prompt 包。**  
   **（已完成：深度分析闭环）** 当前已实现“单个 TXT -> 深度分析任务 -> 完整分析报告 -> 风格摘要 -> 风格母 Prompt 包 -> 风格档案 -> 项目挂载”的可用闭环，包含：
   ...
   - 任务阶段 `classifying_input / analyzing_chunks / aggregating / reporting / summarizing / composing_prompt_pack`
   - 基于已配置 Provider/模型的真实 LLM 多阶段分析与二次 Prompt Composer
   - 返回结果包 `analysis_meta / analysis_report / style_summary / prompt_pack`
   - `Style Lab` 页面支持只读完整报告、可编辑风格摘要、可编辑风格母 Prompt 包
   - 风格档案保存后支持再次覆盖更新当前摘要与 Prompt 包，并继续挂载到项目
   - 对旧版 `draft` 结果提供兼容读取与降级转换
   当前仍未实现多 TXT 合并、外部任务队列、复杂重试/恢复、独立证据账本持久化、LangGraph 编排与超长文本专项优化。
...
- 后端任务结果已从旧 `draft` 草案升级为 `analysis_meta / analysis_report / style_summary / prompt_pack`
...
- LangChain 当前承担模型适配、Provider 连通性测试、分阶段风格分析与风格母 Prompt 包生成
...
当前已落地的是一个**效果优先的简化版深度分析流水线**：

- 上传单个 TXT 后，系统先把原文落到本地存储，再创建数据库任务记录
- Worker 会按 `classifying_input -> analyzing_chunks -> aggregating -> reporting -> summarizing -> composing_prompt_pack` 顺序推进
- 当前切片仍采用段落聚合的启发式策略，但最终产物已从单个草案扩展为“分析元信息 + 结构化报告 + 风格摘要 + 风格母 Prompt 包”
- 完整分析报告为只读审阅层；风格摘要和 Prompt 包为可编辑层
- 风格档案保存后，项目侧后续消费的是当前风格资产上的 `prompt_pack`，而不是旧的 `draft` 草案
- 这套实现优先保证“分析更严、产物更可复用、写作侧可直接消费”的闭环，而不是一步到位实现完整 LangGraph 图
...
当前数据库中实际落地的风格档案对象，已经从“草案字段集合”升级为“报告 + 摘要 + Prompt 包”的组合体，更接近下面的形态：
...
```json
{
  ...
  "analysis_report": {
    "executive_summary": "...",
    "basic_assessment": "...",
    "sections": ["3.1", "3.2", "...", "3.12"],
    "appendix": "..."
  },
  "style_summary": {
    "style_positioning": "整体语言古雅克制，动作场面偏短句，节奏紧凑。",
    "core_features": ["短句推进", "冷感意象", "对白克制"],
    "scene_strategies": [{"scene": "dialogue", "instruction": "对白尽量简短，不要现代口头禅。"}],
    "generation_notes": ["优先保留高置信特征。"]
  },
  "prompt_pack": {
    "system_prompt": "以克制、古雅、动作精准为核心约束进行生成。",
    "scene_prompts": {
...
- `prompt_pack.hard_constraints`
- `prompt_pack.few_shot_slots`
...
- 后端 pytest 回归：初始化、登录、登出、Session 校验、Provider CRUD、Provider 测试、项目 CRUD、归档/恢复、Alembic 迁移  
  **（已完成）**
- 后端 Style Lab 回归：任务创建、非法文件与空文件校验、输入判定、分块分析聚合、报告/摘要/Prompt 包产出、陈旧任务失败处理、风格档案保存与覆盖更新、项目挂载  
  **（已完成：深度分析闭环）**
...
- Style Lab 深度分析结果包：输入判定、分块分析、结构化报告、风格摘要、风格母 Prompt 包  
  **（已完成：深度分析闭环）**
- Style Lab 的下一阶段增强：多 TXT 合并、独立证据账本持久化、超长文本批处理、任务恢复/重试、分析质量与聚合策略升级  
  尚未开始
