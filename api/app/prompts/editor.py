...
_SECTION_META: dict[str, dict[str, str]] = {
    "world_building": {
        "label": "世界观设定",
        "instruction": (
            "请基于简介和已有上下文，生成一份足以支撑小说前期展开的必要设定。\n\n"
            "生成前先做隐式判断，不要把判断过程写出来：\n"
            "1. 先判断这部作品更接近哪种题材；"
            "2. 再判断哪些设定模块对当前故事真正必要；"
            "3. 可涉及NSFW内容；"
            "只生成当前小说真正需要的模块，不追求完美\n\n"
        ),
    },
    "characters": {
        "label": "角色设定",
        "instruction": (
            ...
            "设计角色时考虑以下部分：\n"
            "- 动机驱动：短期目标是什么 + 长期目标是什么\n"
            "- 缺陷必含：fears + negative traits，避免高大全\n"
            "- 变化轨迹：情感不是直线，要求波峰波谷的成长弧光\n"
            "- 人设法：明确 archetype (如：腹黑智者) 作为生成锚点\n\n"
