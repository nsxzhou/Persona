...
def build_sketch_prompt(
    *,
    ...
    return (
        ...
        "只记录当前 chunk 的直接证据：当前 chunk 没有发生、没有出场、没有明确点名的内容，不得写入 `events` 或 `characters_present`。\n"
...
def build_skeleton_reduce_prompt(
    *,
    ...
    return (
        ...
        "阶段边界必须解释为什么在这些 chunk 发生切换；不要只按平均长度切段。\n"
        "主线推进链请优先写成“设伏 @chunkX -> 兑现 @chunkY”的可追踪链条。\n"
        "角色能力阶梯必须区分登场、升级、关系功能变化，避免把所有角色都压成同一种功能位。\n"
...
def build_skeleton_group_reduce_prompt(
    *,
    ...
    return (
        ...
        "阶段边界必须解释为什么在这些 chunk 发生切换；主线推进链优先写成“设伏 @chunkX -> 兑现 @chunkY”；"
        "角色能力阶梯必须区分登场、升级、关系功能变化。\n\n"
        f"输入判定：{json.dumps(classification, ensure_ascii=False)}\n"
    ...
    *,
    ...
    if skeleton_block:
        extra_hint = (
            ...
            "骨架的阶段与节奏判断。每个 3.x 章节都必须收束为可复用的情节机制，而不是事件流水账。\n\n"
        )
    return (
        f"{SHARED_ANALYSIS_RULES}\n\n"
...
def build_story_engine_prompt(
    *,
    ...
    return (
        ...
        "- suggested_overlays 是最低优先级兼容字段，只输出倾向推荐，不输出执行指令；允许的 overlay 名称：`harem_collect`、`wife_steal`、`reverse_ntr`、`hypnosis_control`、`corruption_fall`、`dominance_capture`。\n"
        "- 不得让 overlay 倾向主导 Story Engine；如果证据不足，`suggested_overlays` 可以为空或只写最少项。\n\n"
        "抽象要求：\n"
        "- 每个字段都必须写成情节执行指纹：可复用、可落地、能指导后续生成，而不是泛泛评价。\n"
        "- 主驱动轴不是题材标签，而是“压力如何制造 -> 行动如何升级 -> 兑现如何形成追读”的推进机制。\n"
