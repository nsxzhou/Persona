...
6. 分析结论必须最终能转化为可执行写作资产：句法、节奏、词汇、标点、对白、意象和叙述视角必须尽量落到操作规则，而不是停在审美评价。
""".strip()
...
def build_chunk_analysis_prompt(
    *,
    ...
    return (
        ...
        "每个要点优先采用“风格现象 -> 可复用写法 -> 证据摘要”的结构：先说文本怎么写，"
        "再说明后续生成时可以如何执行，最后用样本证据压缩支撑；不要只写“有画面感、细腻、节奏好”等空泛判断。\n\n"
...
def build_report_prompt(
    *,
    ...
    return (
        ...
        "报告既要给人审阅，也要服务后续 Voice Profile 抽象；每个风格维度都要尽量保留“证据账本”和“可执行写法”两层，"
        "避免只剩文学评论式总结。\n\n"
        f"输入判定：{json.dumps(classification, ensure_ascii=False)}\n"
        f"输出模板：\n{REPORT_TEMPLATE}\n\n"
