...
@pytest.mark.asyncio
async def test_get_plot_status_confirms_unacknowledged_pause_after_timeout(
    initialized_client: AsyncClient,
    app_with_db: FastAPI,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    create_response = await initialized_client.post(
        "/api/v1/plot-analysis-jobs",
        data={"plot_name": "暂停确认", "provider_id": provider_id},
        files={"file": ("sample.txt", "第一段。".encode("utf-8"), "text/plain")},
    )
    job_id = create_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_plot_status_keeps_running_when_worker_heartbeats_after_pause_request(
    initialized_client: AsyncClient,
    app_with_db: FastAPI,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    create_response = await initialized_client.post(
        "/api/v1/plot-analysis-jobs",
        data={"plot_name": "活跃暂停确认", "provider_id": provider_id},
        files={"file": ("sample.txt", "第一段。".encode("utf-8"), "text/plain")},
    )
    job_id = create_response.json()["id"]
