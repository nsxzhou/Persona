...
export function AppShell({ children }: PropsWithChildren) {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 text-foreground">
      <aside className="motion-nav fixed inset-y-0 z-10 flex w-64 flex-col border-r border-border bg-background">
        <div className="motion-nav flex h-16 items-center px-6 border-b border-border">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-4">
          {NAV_ITEMS.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href}
              className="motion-button flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-accent-foreground hover:translate-x-1 border-l-4 border-transparent hover:border-primary"
            >
      ...
      <main className="motion-page ml-64 flex-1 space-y-10 p-8">
        {children}
      </main>
    </div>
  );
