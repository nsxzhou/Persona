...
function getFlatItemKey(item: FlatItem): string {
  ...
  if (item.type === "empty-volume") return `empty-volume-${item.vi}`;
  if (item.type === "chapter") return `chapter-${item.vi}-${item.ci}`;
  return `chapter-details-${item.vi}-${item.ci}`;
}
