...
type LabNewTaskDialogProps<TPayload> = {
  ...
  createJob: (payload: TPayload) => Promise<{ id: string }>;
  buildPayload: (values: CreateLabTaskFormValues) => TPayload;
};

const fileSchema = z.custom<File | null>((value) => value instanceof File, {
  message: "请上传 TXT 样本",
});
...
export function LabNewTaskDialog<TPayload>({
  providers,
  nameLabel,
  namePlaceholder,
  nameRequiredMessage,
  ...
  createJob,
  buildPayload,
}: LabNewTaskDialogProps<TPayload>) {
  ...
  const form = useForm<CreateLabTaskFormValues>({
  ...
  const createJobMutation = useMutation({
    ...
    onSuccess: (newJob) => {
      ...
      setOpen(false);
      resetForm();
      router.push(`${redirectBasePath}/${newJob.id}`);
    },
    onError: (error) => {
      ...
      <DialogContent className="sm:max-w-[600px]">
        <form
          onSubmit={form.handleSubmit((values) => {
            ...
            createJobMutation.mutate(buildPayload(values));
          })}
        >
          <DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor={`${inputIdPrefix}-name-input`}>{nameLabel}</Label>
              <Input
                id={`${inputIdPrefix}-name-input`}
                placeholder={namePlaceholder}
                {...form.register("name")}
              />
              {nameError ? <p className="text-xs text-destructive">{String(nameError)}</p> : null}
            </div>
            <div className="grid gap-2">
              <Label htmlFor={`${inputIdPrefix}-provider-select`}>Provider</Label>
              <Controller
                control={form.control}
                name="provider_id"
                render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange}>
                    <SelectTrigger id={`${inputIdPrefix}-provider-select`}>
            ...
            <div className="grid gap-2">
              <Label htmlFor={`${inputIdPrefix}-model-override`}>模型覆盖 (选填)</Label>
              <Input
                id={`${inputIdPrefix}-model-override`}
                placeholder="留空则使用 Provider 默认模型"
                {...form.register("model")}
              />
            </div>
            <div className="grid gap-2">
              <Label htmlFor={`${inputIdPrefix}-sample-file`}>TXT 样本</Label>
              <Controller
                control={form.control}
                name="file"
                render={({ field }) => (
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={createJobMutation.isPending || !canSubmit}>
