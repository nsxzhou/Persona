...
const createTaskSchema = z.object({
  plot_name: z.string().trim().min(1, "请输入情节档案名称"),
...
export function PlotLabNewTaskDialog({ providers }: { providers: ProviderConfig[] }) {
  ...
  const createJobMutation = useMutation({
    mutationFn: api.createPlotAnalysisJob,
    onSuccess: (newJob) => {
      ...
      router.push(`/plot-lab/${newJob.id}`);
    },
    onError: (error) => {
        ...
        <form
          onSubmit={form.handleSubmit((values) => {
            ...
            createJobMutation.mutate({
              plot_name: values.plot_name.trim(),
        ...
        >
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="style-name-input">情节档案名称</Label>
              <Input
                id="style-name-input"
                placeholder="例如：反派修罗场模板"
                {...form.register("plot_name")}
              />
              {form.formState.errors.plot_name ? (
                <p className="text-xs text-destructive">{form.formState.errors.plot_name.message}</p>
              ) : null}
            </div>
            <div className="grid gap-2">
