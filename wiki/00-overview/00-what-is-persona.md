...
- **蓝图/活态双层架构**：`projects` 表上五个蓝图字段（AI 不自动改）+ 两个运行时字段（AI 在续写后提议更新），差异通过 Diff Dialog 人肉确认
- **风格档案挂载**：项目可挂载一个已保存的 Style Profile，`prompt_pack_markdown` 自动注入所有写作请求
...
3. **项目管理 CRUD**：创建、详情、编辑、归档、恢复、挂载 Style Profile、导出 txt/epub。详见 [20 项目](../20-domains/20-projects.md)。
...
sequenceDiagram
    ...
    participant Worker as Style Analysis Worker
    ...
    API->>Worker: (独立进程 claim 任务)
    Worker->>LLM: LangGraph pipeline 多阶段调用
    LLM-->>Worker: Markdown 报告 / 摘要 / Prompt Pack
