...
Persona 前端用一套**明确的边界约定 + 薄 API 客户端 + TanStack Query 做缓存**把这些问题收口。本章给出边界约束、数据流、常见坑，与 `.trellis/spec/frontend/` 的硬规则对齐。
...
### RSC 边界规则（`.trellis/spec/frontend/component-guidelines.md`）
...
### Server Actions 与 TanStack Query 的协同（`.trellis/spec/frontend/state-management.md`）
...
### OpenAPI 作为单一事实源（`.trellis/spec/frontend/type-safety.md`）
...
`.trellis/spec/frontend/state-management.md` 明确：处理表单时**优先用**这三个，不要退回 React 18 的 `useState` 样板。

当前代码里**使用 TanStack Query `useMutation` 接管**绝大多数表单 loading 态（`mutation.isPending` / `mutation.error`），因此这三个新 Hook 的直接出现不多。一旦新增一个**不需要缓存失效**的表单，优先用 `useActionState` + `useFormStatus` 组合。Server Action 内部**必须**用 Zod 二次校验入参（`.trellis/spec/frontend/state-management.md` 末尾）。

### 样式系统（`.trellis/spec/frontend/component-guidelines.md`）
...
| 手写 interface 和生成类型并存 | 违反 Trellis frontend type-safety spec 双轨禁令 | 删手写；用 `OpenApiSchema<"...">` 派生 |
