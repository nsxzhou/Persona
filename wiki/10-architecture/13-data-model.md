...
Relationship：`projects`, `style_analysis_jobs`, `style_profiles` 都引用它（作为 `default_provider` / `provider`）。`api_key_hint` 是 Python 属性（`f"****{last4}"`），不是列。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `runtime_state` | Text | 当前世界状态（谁在哪、事件发生到哪） |
| `runtime_threads` | Text | 情节线程（未了结的伏笔、悬念） |
...
1. **定义 Model**（`api/app/db/models.py` 文件末尾）
   ```python
...
3. **生成迁移**
   ```bash
   cd api
   uv run alembic revision --autogenerate -m "add comments"
   ```
4. **Review 迁移文件**：检查外键、索引、默认值跨方言兼容
5. **跑迁移**
   ```bash
   uv run alembic upgrade head
   ```
6. **后续层**：Schema / Repository / Service / Router（见 [11 后端分层](./11-backend-layering.md) "新增业务领域"）
