...
初始化入口是 `api/app/api/routes/setup.py`：

- `GET /api/v1/setup/status` 只回答系统是否已初始化
- `POST /api/v1/setup` 会一次性完成三件事：创建管理员、创建首个 Provider、写入 Session Cookie

真正的“只能初始化一次”由 `AuthService.ensure_not_initialized()` 保证，见 `api/app/services/auth.py:52`。一旦 `users` 表里已有数据，再次 setup 会直接返回冲突错误。

### 登录与登出是标准 HttpOnly Session

登录入口在 `api/app/api/routes/auth.py`：

- 路由层调用 `AuthService.authenticate()` 校验用户名和密码，见 `api/app/services/auth.py:56`
- 认证成功后调用 `AuthService.create_session()` 创建一条 `sessions` 记录，见 `api/app/services/auth.py:68`
- `set_session_cookie()` 把 raw token 写入浏览器 Cookie，见 `api/app/api/deps.py:28`

登出入口在 `api/app/api/routes/auth.py`：

- 先从请求 Cookie 取 raw token
- Service 层按哈希删除 session 记录
- 最后返回 `delete_cookie()` 清掉浏览器端状态

`GET /api/v1/auth/me` 位于 `api/app/api/routes/auth.py`，本身几乎没有业务逻辑，它的意义是证明 `CurrentUserDep` 已经把认证做完。

### 账号注销与跨模块清理

账号注销（`AuthService.delete_account()`，见 `api/app/services/auth.py:122`）不仅会删除用户表及关联数据，还负责清理外部存储资源（如风格分析的样章文件、大纲推演的参考文件、以及它们生成的各种工件目录）。

它会：
- 先从数据库拉取待清理的文件路径和 Job ID
- 同步删除所有的数据库账户数据
- 使用 `asyncio.to_thread` 容错式删除散落的独立文件（不会因为文件找不到阻塞整个注销流程）
- 使用 `asyncio.TaskGroup` 与并发信号量（Semaphore）并发调用外部模块（`StyleAnalysisStorageService` 和 `PlotAnalysisStorageService`）的复杂清理逻辑
...
- `hash_session_token()` 用 HMAC-SHA256 把 raw token 转成可持久化的哈希值，入口在 `api/app/core/security.py:56`。它会优先使用 `settings.session_secret`，如果未配置则回退到 `settings.encryption_key` 作为 HMAC 的密钥。
...
`api/app/api/deps.py:28` 的 `set_session_cookie()` 统一设置：
...
`api/app/api/deps.py:48` 的 `get_current_user()` 是真正的认证门：

- 从请求 Cookie 取 `persona_session`
- 调用 `AuthService.resolve_user_by_token()`
- 返回 `User` ORM 对象

`api/app/api/deps.py:60` 把它包装成 `CurrentUserDep` 之后，所有业务路由都只要声明一个依赖即可自动变成“必须登录”接口。典型例子见：
...
| 症状 | 常见原因 | 先看哪里 |
| --- | --- | --- |
| `/projects` 一直跳回 `/login` | Cookie 没写入、Cookie 名不匹配、CORS 未允许凭证 | `api/app/api/deps.py:28`、`api/app/main.py:75` |
| setup 提交成功但刷新后又回 `/setup` | setup 成功后没有正确失效 `setup-status` 缓存 | `web/components/route-guards.tsx` |
| 数据库里有 `sessions` 记录但 `/auth/me` 仍返回未登录 | 浏览器存的是 raw token，DB 里查的是 HMAC；确认哈希逻辑一致 | `api/app/core/security.py:56`、`api/app/services/auth.py:82` |
| 读请求频繁更新 `sessions.last_accessed_at` | `resolve_user_by_token()` 的节流逻辑被破坏 | `api/app/services/auth.py:101` |
