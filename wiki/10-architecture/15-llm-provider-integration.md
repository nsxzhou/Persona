...
页面入口在 `web/app/(workspace)/settings/models/page.tsx`，主要组件链路是：

- `web/components/provider-configs-page-view.tsx` 负责 list / test / delete / 打开表单
- `web/components/provider-config-form-dialog.tsx` 负责 create/edit 弹窗
- `web/components/provider-form-fields.tsx` 负责字段复用
- `web/lib/validations/provider.ts` 统一定义 Zod schema 与默认值

从当前 schema 看，这一页暴露的核心字段是：
