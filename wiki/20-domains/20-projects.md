...
3. 创建后进入项目详情页，在工作台内编辑描述、默认 Provider、默认模型与挂载的 Style Profile。
...
项目详情页本身是一个 Server Component：`web/app/(workspace)/projects/[id]/page.tsx:5`。它会先在服务端并发拉取：

- 项目本体
- Provider 列表
- Style Profile 列表

然后把这些数据一次性喂给 `ProjectWorkbench`。
...
- `create()` 会先校验默认 Provider 已启用，必要时校验 `style_profile_id` 存在，见 `api/app/services/projects.py:55`
- `update()` 会处理 Provider 切换、模型兜底、Style Profile 重挂载与可编辑字段白名单，见 `api/app/services/projects.py:100`
...
核心表是 `api/app/db/models.py:127` 的 `Project`：

- 元数据：`name`、`description`、`status`、`archived_at`
- 依赖关系：`default_provider_id`、`default_model`、`style_profile_id`
...
项目本身不存章节正文；正文分拆到 `project_chapters`。
...
- `style_profile_id` 决定 `prompt_pack_payload` 是否会被注入到写作系统提示词中
