...
4. Worker 继续驱动 LangGraph 把骨架注入到后续的报告整理阶段。
...
- `web/hooks/use-plot-lab-wizard-logic.ts:129` 汇总 report / **skeleton** / story engine 的加载，只有任务成功且尚未保存为 Profile 时才启用
...
- `GET /plot-analysis-jobs/{id}/story-engine`，见 `api/app/api/routes/plot_analysis_jobs.py:187`
...
- `build_report`（直接使用骨架进行报告整理），见 `api/app/services/plot_analysis_pipeline.py:521`
- `build_story_engine`，见 `api/app/services/plot_analysis_pipeline.py:566`
- `persist_result`，见 `api/app/services/plot_analysis_pipeline.py:588`

*注：当前 Plot Lab 管道跳过了 `analyze_chunk` 和 `merge_chunks` 节点，直接从 `build_skeleton` 流转到 `build_report`。*
...
- Checkpointer 工厂使用 `api/app/services/checkpointer_factory.py` 的 `PlotAnalysisCheckpointerFactory`，暂时复用 `PERSONA_STYLE_ANALYSIS_CHECKPOINT_URL`
- Plot 专用切块与 manifest 在 `api/app/services/plot_analysis_text.py`，编码探测和通用分类类型来自 `api/app/core/text_processing.py`
- LLM 客户端与 Markdown 重试逻辑复用 `api/app/services/llm_provider.py` 的 `LLMProviderService.invoke_markdown_completion()`

### Checkpointer 与落盘产物

- Checkpointer 工厂在 `api/app/services/checkpointer_factory.py`
...
3. `selecting_focus_chunks`（当前跳过）
4. `analyzing_focus_chunks`（当前跳过）
5. `aggregating`（当前跳过）
6. `reporting`
7. `postprocessing`
...
- `PLOT_REPORT_TEMPLATE` / `PLOT_STORY_ENGINE_TEMPLATE` / `PLOT_SKELETON_TEMPLATE` 把最终输出格式全部锁死
...
| `build_chunk_analysis_prompt()` | `api/app/prompts/plot_analysis.py` | analyze_chunk（当前跳过） | `plot_skeleton: str \| None` |
| `build_merge_prompt()` | `api/app/prompts/plot_analysis.py` | merge_chunks（当前跳过） | `plot_skeleton: str \| None` |
...
`api/app/services/llm_provider.py` 的 `LLMProviderService.invoke_markdown_completion()`（按任务解析 temperature、空响应重试、从多个位置兜底提取正文）被 Plot 流水线原样复用。

### 输入判定在 Worker 前置

`api/app/services/plot_analysis_text.py:422` 的 `read_plot_chunks_and_classification()` 在真正跑图前完成文本清洗、Plot 专用 chunk 切分与输入分类（章节正文 / 口语字幕 / 混合文本）；编码探测复用 `api/app/core/text_processing.py`。这些 classification 结果会直接注入到 sketch / 分析 / Reduce / 报告四条 prompt 里。
...
flowchart TD
    ...
    Skeleton --> Report["build_report（骨架感知）"]
...
- `api/app/services/checkpointer_factory.py`（包含 Plot Checkpointer 工厂）
- `api/app/services/plot_analysis_jobs.py`
- `api/app/api/routes/plot_analysis_jobs.py`
- `api/app/schemas/plot_analysis_jobs.py`
- `api/app/services/llm_provider.py`（共享 LLM 客户端）
- `api/app/core/text_processing.py`（共享编码探测与分类类型）
- `api/app/services/plot_analysis_text.py`（Plot 专用 chunking + classification）
