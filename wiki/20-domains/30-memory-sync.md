...
- `memory_sync_status`: `checking / pending_review / synced / no_change / failed`
- `memory_sync_source`: `manual / auto`
- `memory_sync_scope`: `generated_fragment / chapter_full`
- `memory_sync_proposed_state` / `memory_sync_proposed_threads` / `memory_sync_proposed_characters_status` / `memory_sync_proposed_summary`: 待确认版本（分别对应运行时状态、伏笔追踪、人物状态、剧情摘要）
- `memory_sync_checked_at`: 记录最后一次检查的时间
- `memory_sync_checked_content_hash`: 记录检查时的正文内容哈希，一旦正文发生改变可清空或使缓存失效
- `memory_sync_error_message`: 记录同步失败时的错误详情
