...
完整报告适合审阅，但不适合每次生成都整份注入。于是有了 `build_style_summary_prompt()`，定义在 `api/app/prompts/style_analysis.py`。
...
最终执行资产由 `build_prompt_pack_prompt()` 生成，定义在 `api/app/prompts/style_analysis.py`。它把摘要组织成：
...
它不是“作品设定摘要”或“角色卡摘要”：样本中的人物名、势力名、事件名、专属设定词都应在这一层被去样本化，改写为可跨作品复用的角色原型、冲突原型和语义类别。
如果样本里存在非常强的风格锚点，例如固定叙事人称、特定题材语境或高频思维模板，也只能在证据稳定时写成“优先采用的偏好”，不能提升成所有项目都必须满足的硬约束。
