...
- `build_chunk_analysis_prompt()`：`api/app/services/style_analysis_prompts.py:110`
- `build_merge_prompt()`：`api/app/services/style_analysis_prompts.py:129`
- `REPORT_TEMPLATE`：`api/app/services/style_analysis_prompts.py:22`
...
完整报告适合审阅，但不适合每次生成都整份注入。于是有了 `build_style_summary_prompt()`，见 `api/app/services/style_analysis_prompts.py:159`。
...
### 第四步：再把摘要转成 Prompt Pack

最终执行资产由 `build_prompt_pack_prompt()` 生成，见 `api/app/services/style_analysis_prompts.py:174`。它把摘要组织成：
...
Style Lab 把“风格摘要”和“Prompt Pack”设计成可编辑层，前端保存链路见 `web/components/style-lab-profile-view.tsx:17` 与 `web/hooks/use-style-lab-wizard-logic.ts:281`。

这意味着作者不必接受模型原样产物：

- 报告可以只读保留作证据账本
- 摘要可以人工去噪
- Prompt Pack 可以继续人工修辞或删改硬约束
...
| 风格摘要 | `StyleAnalysisPipeline._build_summary()` |
| Prompt Pack | `StyleAnalysisPipeline._build_prompt_pack()` |
...
| Prompt Pack 太长太散 | 把报告原样压进去了，没有做第二次抽象 | 先重写摘要，再生成 Prompt Pack |
| 报告很长但没法复用 | 没有把“结论”和“证据账本”分层 | 保留报告只读，把操作性内容收进摘要与 Pack |
