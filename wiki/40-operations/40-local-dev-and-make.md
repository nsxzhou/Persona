...
`Makefile` 的 `dev` 目标把本地开发环境拆成 `db -> api -> worker -> web -> status` 五段：
...
- `status`：回显数据库 / API / Worker / Web 四类状态

根 README 也把 `make dev` 列为推荐方式，见其“快速开始”一节。

### 首次启动会自动补依赖与 env 文件

Makefile 中有两个“偷懒但实用”的设计：

- 如果 `api/.env` 不存在，就从 `api/.env.example` 复制
- 如果 `web/.env.local` 不存在，就从 `web/.env.local.example` 复制
...
如果不想走 Makefile，也可以按根 README 的四段手动启动：

- 数据库：根 README 的“1. 启动数据库”
- 后端：根 README 的“2. 启动后端”
- Worker：根 README 的“3. 启动 Worker”
- 前端：根 README 的“4. 启动前端”
